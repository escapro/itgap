<main class="content">
	<div class="block page-right_block">
		<h1 class="section-head__title">Контакты</h1>
		<div class="post-entry">
			<p>По каким-либо вопросам: <a href="mailto:info@itgap.ru">info@itgap.ru</a></p>
			<hr>
			<p>Мы в Вконтакте: <a href="https://vk.com/itgap_official" target="_blank">https://vk.com/itgap_official</a></p>
			<p>Мы в Twitter: <a href="https://twitter.com/itgap_official" target="_blank">https://twitter.com/itgap_official</a></p>
			<p>Мы в Telegram: <a href="https://t.me/itgap_official" target="_blank">https://t.me/itgap_official</a></p>
			<p>Мы в Дзене: <a href="https://zen.yandex.ru/itgap" target="_blank">https://zen.yandex.ru/itgap</a></p>
		</div>
	</div>
</main>