...
class Post_model extends CI_Model {
	...
	// Отправка поста на модерацию(Сохранить id поста в таблицу considered_posts)
	...
	public function get_suggest_posts($limit) {
		$this->db->select("p.id as post_ID, p.title, p.preview_text, p.post_id as post_id, p.last_change, p.preview_image_url as image_url, p.post_name, cat.url_name as category_url, pv.count as views");
		$this->db->from("posts p");
		$this->db->join('active_posts a', 'p.id=a.post_id');
		$this->db->join('categories cat', 'cat.id=p.category_id');
		$this->db->join('post_views pv', 'pv.post_id=p.id');
